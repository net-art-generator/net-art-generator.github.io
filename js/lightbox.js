...
var Lightbox = {
  ...
  init: function(options) {
    ...
    this.prevLink = new Element('a').setProperties({
      id: 'lbPrevLink',
      href: '#'
    }).setStyle('display', 'none').injectInside(this.controlDiv);
  ...
  changeImage: function(imageNum) {
    ...
    if (this.options.animateCaption.In && this.bottom.offsetHeight) {
      ...
      this.bottom.effect('margin-top', {
        duration: 300,
        onComplete: this.loadImage.bind(this)
      }).start(-this.bottom.offsetHeight);
    } else {
  ...
  nextEffect: function() {
    switch (this.step++) {
      ...
      case 3:
        ...
        if (this.options.descriptions != false) {
          extra += this.description.getStyle('height').toInt()
        }
        if (this.options.showControls) {
          extra += this.controlDiv.getStyle('height').toInt()
        }
        if (this.options.showNumbers) {
          extra += this.number.getStyle('height').toInt()
        }
